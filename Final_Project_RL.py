...
def train_q_learning(grid_size, start_state, end_state, obstacles, total_episodes, learning_rate, discount_factor, epsilon_decay_rate, min_epsilon, progress_callback=None, distance_weights=None, distance_scale=1.0):
    """Runs the Q-Learning training process.
    If distance_weights is provided (real-world mode), each step's reward is based on
    the real-world distance covered instead of a flat -1 -- see get_reward_weighted.
    Original Grid mode leaves distance_weights=None and behaves exactly as before."""
    ...
    for episode in range(total_episodes):
        ...
        while not done and step_count < max_steps_per_episode:
            ...
            if distance_weights is not None:
                reward = get_reward_weighted(current_state, next_state, end_state, obstacles, distance_weights, distance_scale)
            else:
...
def choose_epsilon_greedy_action(q_table, row, col, epsilon):
    """Shared epsilon-greedy action selection used by Q-learning and SARSA."""
    if random.uniform(0, 1) < epsilon:
...
def train_sarsa(grid_size, start_state, end_state, obstacles, total_episodes, learning_rate, discount_factor, epsilon_decay_rate, min_epsilon, progress_callback=None, distance_weights=None, distance_scale=1.0):
    """Runs on-policy SARSA training. Structurally mirrors train_q_learning so the two
    can be compared fairly on identical grids/hyperparameters -- the only difference is
    the update rule (uses the actually-taken next action instead of max over next actions)."""
    if not grid_size or start_state is None or end_state is None or obstacles is None:
        print("Cannot train (SARSA): Invalid grid parameters.")
        return None, []
    ...
    if start_state in obstacles or end_state in obstacles:
        messagebox.showerror("Training Error", "Start or End position cannot be an obstacle.")
        return None, []

    if not has_path(grid_size, start_state, end_state, obstacles):
        messagebox.showerror("Training Error", "No possible path exists between start and end with current obstacles.")
        return None, []

    for episode in range(total_episodes):
        current_state = start_state
        row, col = current_state
        action_index = choose_epsilon_greedy_action(q_table, row, col, epsilon)
        ...
        while not done and step_count < max_steps_per_episode:
            ...
            next_row, next_col = next_state
            # SARSA is on-policy: pick the NEXT action now, and use ITS Q-value in the
            # update (not the max over next-state actions, which is what makes this
            # different from Q-learning).
            next_action_index = choose_epsilon_greedy_action(q_table, next_row, next_col, epsilon)

            old_value = q_table[row, col, action_index]
            next_value = q_table[next_row, next_col, next_action_index]
...
def astar_path(grid_size, start, end, obstacles):
    """Classical A* search -- used as a 'ground truth optimal path' baseline to compare
    against the paths learned by Q-learning and SARSA. Uses octile distance as the
    heuristic since ACTIONS includes diagonal moves."""
    import heapq
    ...
    open_heap = [(octile_heuristic(start, end), 0.0, start)]
    came_from = {}
    g_score = {start: 0.0}
    visited = set()
    ...
    while open_heap:
        ...
        for dr, dc in ACTIONS:
            ...
            if not is_valid_state(neighbor, grid_size, obstacles_set):
                continue
            tentative_g = g + step_cost(dr, dc)
            if tentative_g < g_score.get(neighbor, float('inf')):
                g_score[neighbor] = tentative_g
                came_from[neighbor] = current
                f_score = tentative_g + octile_heuristic(neighbor, end)
...
def haversine_distance(lon1, lat1, lon2, lat2):
    ...
    OSMnx graph nodes store 'x' (longitude) and 'y' (latitude) in degrees, so this is
    the right distance to use for real-world edge weights (straight-line, not
    routed-along-roads, but far more meaningful than a flat 'one grid step' cost)."""
...
def compute_grid_distance_weights(grid_size, obstacles, grid_to_node_map, graph):
    """Computes the real-world (haversine) distance in meters between every pair of
    adjacent, non-obstacle grid cells. Returns (weights_dict, mean_distance) where
    weights_dict maps (cell_a, cell_b) -> meters for BOTH directions (symmetric)."""
    obstacles_set = set(obstacles)
    weights = {}
    distances_seen = []
    ...
    for row in range(grid_size):
        for col in range(grid_size):
            cell = (row, col)
            if cell in obstacles_set or cell not in grid_to_node_map:
                continue
            node1 = grid_to_node_map[cell]
            lon1, lat1 = graph.nodes[node1]['x'], graph.nodes[node1]['y']

            for dr, dc in ACTIONS:
                neighbor = (row + dr, col + dc)
                if (cell, neighbor) in weights:
                    continue  # already computed from the other direction
                if not (0 <= neighbor[0] < grid_size and 0 <= neighbor[1] < grid_size):
                    continue
                if neighbor in obstacles_set or neighbor not in grid_to_node_map:
                    continue

                node2 = grid_to_node_map[neighbor]
                lon2, lat2 = graph.nodes[node2]['x'], graph.nodes[node2]['y']
                dist = haversine_distance(lon1, lat1, lon2, lat2)

                weights[(cell, neighbor)] = dist
                weights[(neighbor, cell)] = dist
                distances_seen.append(dist)

    mean_distance = float(np.mean(distances_seen)) if distances_seen else 1.0
    return weights, mean_distance


def get_reward_weighted(current_state, next_state, end_state, obstacles, distance_weights, distance_scale=1.0):
    """Distance-aware reward: instead of a flat -1 per step (which only rewards fewer
    grid hops), penalize proportionally to the real-world distance covered. This makes
    the agent optimize for real-world path length, not just cell-count."""
    if next_state == end_state:
        return GOAL_REWARD
    elif next_state in obstacles:
        return OBSTACLE_REWARD
    else:
        dist = distance_weights.get((current_state, next_state))
        if dist is None:
            # No edge-weight on record for this pair (e.g. the move was invalid and the
            # agent stayed put) -- fall back to the flat step penalty.
            return DEFAULT_REWARD
...
class App:
    ...
    def show_original_grid_interface(self):
        ...
        ttk.Button(config_frame, text="Compare Algorithms", command=self.run_algorithm_comparison).grid(row=0, column=3, rowspan=2, padx=5, pady=2)
    ...
    def show_real_world_map_interface(self):
        ...
        # Toggle: use real-world (haversine) distance between mapped points as the step
        # cost, instead of a flat -1 per grid hop. This makes the agent optimize for
        # actual real-world path length rather than just fewest grid cells.
        self.use_real_distance_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(place_frame, text="Use real-world distances as edge weights",
                        variable=self.use_real_distance_var).grid(row=1, column=0, columnspan=2, sticky="w", padx=5, pady=(4, 0))
        ...
        # Real-world path distance summary (populated after training)
        self.path_distance_label_rw = ttk.Label(self.real_world_frame, text="", font=("Arial", 10, "bold"))
        self.path_distance_label_rw.pack(pady=2)
    ...
    def run_algorithm_comparison(self):
        """Runs Q-learning, SARSA, and A* on the same grid/hyperparameters and shows
        a side-by-side comparison: learning curves for the two RL methods, plus a bar
        chart of path length and a summary table (path length, steps, training time)."""
        # Stop any ongoing animation first, same guard as run_original_grid_training
        self.animation_stop_event.set()
        if self.animation_thread and self.animation_thread.is_alive():
        ...
        try:
            ...
            max_allowed_obstacles = (grid_size * grid_size) // 2
            if not (0 <= num_obstacles <= max_allowed_obstacles):
                raise ValueError(f"Number of obstacles cannot exceed {max_allowed_obstacles}.")
        ...
        # Use the currently generated grid if one exists (so comparison matches what's
        # on screen); otherwise generate a fresh solvable one.
        if self.current_grid_params:
            grid_size, start_state, end_state, obstacles = self.current_grid_params
        else:
            start_state = (0, 0)
            ...
            obstacles = self.generate_obstacles(grid_size, num_obstacles, start_state, end_state)
            if not has_path(grid_size, start_state, end_state, obstacles):
                messagebox.showerror("Grid Generation Error", "Could not generate a solvable grid. Try fewer obstacles.")
                return
            self.current_grid_params = (grid_size, start_state, end_state, obstacles)
            self.draw_grid_on_canvas(grid_size, start_state, end_state, obstacles)
            self.last_drawn_grid_params = self.current_grid_params
        ...
        def comparison_thread():
            self.root.after(0, lambda: messagebox.showinfo(
                "Comparison Started",
                "Running Q-Learning, SARSA, and A* on the same grid. This may take a moment."))

            results = {}

            t0 = time.time()
            q_table, q_rewards = train_q_learning(
                grid_size, start_state, end_state, obstacles, total_episodes,
                learning_rate, discount_factor, epsilon_decay_rate, min_epsilon)
            q_time = time.time() - t0
            q_path = find_optimal_path(q_table, grid_size, start_state, end_state, obstacles) if q_table is not None else []
            results['Q-Learning'] = {'rewards': q_rewards, 'path': q_path, 'time': q_time}
            ...
            sarsa_path = find_optimal_path(sarsa_table, grid_size, start_state, end_state, obstacles) if sarsa_table is not None else []
            results['SARSA'] = {'rewards': sarsa_rewards, 'path': sarsa_path, 'time': sarsa_time}

            t0 = time.time()
            astar_p, astar_cost = astar_path(grid_size, start_state, end_state, obstacles)
            astar_time = time.time() - t0
            results['A*'] = {'rewards': [], 'path': astar_p, 'time': astar_time}

            self.root.after(0, lambda: self.show_comparison_popup(results))

        thread = threading.Thread(target=comparison_thread)
        thread.daemon = True
        thread.start()

    def show_comparison_popup(self, results):
        """Displays learning curves, a path-length bar chart, and a metrics table for
        the Q-Learning vs SARSA vs A* comparison."""
        popup = tk.Toplevel(self.root)
        popup.title("Algorithm Comparison: Q-Learning vs SARSA vs A*")
        popup.geometry("900x750")

        fig, (ax_curves, ax_bar) = plt.subplots(2, 1, figsize=(8, 7), dpi=100)

        # Learning curves (only Q-Learning and SARSA train episodically; A* is a
        # one-shot deterministic search so it has no curve, only a final path length).
        for label in ['Q-Learning', 'SARSA']:
            rewards = results[label]['rewards']
            if rewards:
                ax_curves.plot(rewards, label=label, alpha=0.8)
        ax_curves.set_title("Learning Curves: Q-Learning vs SARSA")
        ax_curves.set_xlabel("Episode")
        ax_curves.set_ylabel("Total Reward")
        ax_curves.legend()
        ax_curves.grid(True)

        # Path length bar chart (steps in path, lower = better; empty path = failed)
        labels = list(results.keys())
        path_lengths = [len(results[k]['path']) - 1 if results[k]['path'] else 0 for k in labels]
        bars = ax_bar.bar(labels, path_lengths, color=['#4C72B0', '#DD8452', '#55A868'])
        ax_bar.set_title("Path Length Comparison (steps, lower is better)")
        ax_bar.set_ylabel("Steps in Final Path")
        for bar, length in zip(bars, path_lengths):
            ax_bar.text(bar.get_x() + bar.get_width() / 2, bar.get_height(), str(length),
                        ha='center', va='bottom')
        ...
        # Summary table
        table_frame = ttk.LabelFrame(popup, text="Summary", padding="10")
        table_frame.pack(fill="x", padx=10, pady=10)

        headers = ["Algorithm", "Path Length (steps)", "Reached Goal?", "Time (s)"]
        for col, h in enumerate(headers):
            ttk.Label(table_frame, text=h, font=("Arial", 10, "bold")).grid(row=0, column=col, padx=8, pady=2, sticky="w")

        for row_idx, label in enumerate(results.keys(), start=1):
            path = results[label]['path']
            steps = len(path) - 1 if path else "N/A"
            reached = "Yes" if path else "No"
            elapsed = f"{results[label]['time']:.3f}"
            ttk.Label(table_frame, text=label).grid(row=row_idx, column=0, padx=8, pady=2, sticky="w")
            ttk.Label(table_frame, text=str(steps)).grid(row=row_idx, column=1, padx=8, pady=2, sticky="w")
            ttk.Label(table_frame, text=reached).grid(row=row_idx, column=2, padx=8, pady=2, sticky="w")
            ttk.Label(table_frame, text=elapsed).grid(row=row_idx, column=3, padx=8, pady=2, sticky="w")

        note = ("Note: A* gives the true shortest path instantly (no learning needed) -- it's "
                "the ground-truth baseline. Q-Learning and SARSA should approach this path "
                "length as training episodes increase; how close they get (and how fast) is "
                "the interesting comparison.")
        ttk.Label(popup, text=note, wraplength=860, justify="left", foreground="gray30").pack(padx=10, pady=(0, 10))
    ...
    def run_real_world_training(self):
        ...
        def real_world_thread():
            ...
            # If enabled, compute real-world (haversine) distances between adjacent grid
            # cells and use them as the step cost during training, instead of a flat -1.
            # We auto-scale so the *average* step penalty matches DEFAULT_REWARD's
            # magnitude -- this keeps reward scale sane regardless of place size or
            # grid resolution, without needing the user to hand-tune a distance_scale.
            use_real_distance = self.use_real_distance_var.get()
            distance_weights = None
            distance_scale = 1.0
            if use_real_distance:
                distance_weights, mean_distance = compute_grid_distance_weights(
                    grid_size_real, obstacles_real, grid_to_node_map, graph)
                if distance_weights and mean_distance > 0:
                    distance_scale = abs(DEFAULT_REWARD) / mean_distance
                else:
                    # No usable edge distances found (shouldn't normally happen) -- fall
                    # back to flat-reward behavior.
                    distance_weights = None
            ...
            if q_table_real is not None:
                ...
                # Report the real-world distance of the learned path (computed regardless
                # of whether distance-weighting was used for training, so you can compare).
                optimal_path_real = find_optimal_path(q_table_real, grid_size_real, start_state_real, end_state_real, obstacles_real)
                total_km = self._compute_path_real_distance_km(optimal_path_real, grid_to_node_map, graph)
                mode_note = "distance-weighted training" if use_real_distance else "flat step-count training"
                if total_km is not None:
                    distance_text = f"Learned path: {len(optimal_path_real) - 1} grid steps ≈ {total_km:.2f} km real-world distance ({mode_note})"
                else:
                    distance_text = f"Learned path: {len(optimal_path_real) - 1} grid steps ({mode_note})"
                self.root.after(0, lambda: self.path_distance_label_rw.config(text=distance_text))
    ...
    def clear_real_world_map_plot(self):
        ...
        if hasattr(self, 'path_distance_label_rw'):
            self.path_distance_label_rw.config(text="")

    def _compute_path_real_distance_km(self, grid_path, grid_to_node_map, graph):
        """Sums haversine distance between each consecutive pair of grid cells in a
        path (using their mapped OSM nodes) to report the real-world path length in km.
        Returns None if the path is empty or cells aren't mapped to graph nodes."""
        if not grid_path or len(grid_path) < 2:
            return None
        total_m = 0.0
        for cell_a, cell_b in zip(grid_path, grid_path[1:]):
            node_a = grid_to_node_map.get(cell_a)
            node_b = grid_to_node_map.get(cell_b)
            if node_a is None or node_b is None:
                continue
            lon1, lat1 = graph.nodes[node_a]['x'], graph.nodes[node_a]['y']
            lon2, lat2 = graph.nodes[node_b]['x'], graph.nodes[node_b]['y']
            total_m += haversine_distance(lon1, lat1, lon2, lat2)
